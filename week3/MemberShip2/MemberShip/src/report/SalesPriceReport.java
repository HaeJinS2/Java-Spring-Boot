...
public class SalesPriceReport {
   ...
   public void generateSaleReport(int price) {
      ...
      BasicSalesRatio bsr = new BasicSalesRatio();
      ratio = bsr.getSalesRatio(price);
      salePrice = price-(int)(price * (ratio * 0.01));
      System.out.println("등급과 가격에 따른 세일 비율은 " + ratio + "이며, 가격은 " + salePrice + "입니다."); 
   
   
      //VIP 등급
      salePrice = price;
      VIPSalesRatio vsr = new VIPSalesRatio();
      ratio = vsr.getSalesRatio(price);
      salePrice = price-(int)(price * (ratio * 0.01));
      System.out.println("등급과 가격에 따른 세일 비율은 " + ratio + "이며, 가격은 " + salePrice + "입니다.");
